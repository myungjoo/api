...
public final class CustomFilter implements AutoCloseable {
     ...
     *
     * @throws IllegalStateException if the custom-filter is still used in a pipeline
     */
    @Override
    public void close() {
        if (mHandle != 0) {
            if (!nativeDestroy(mHandle)) {
                throw new IllegalStateException("Failed to close custom-filter " + mName + ", it is used in a pipeline");
            }
